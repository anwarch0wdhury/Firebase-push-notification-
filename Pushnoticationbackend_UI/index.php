<?php    //You need a server to run it.open xampp local server.copy this files in a folder and put them inside xampp/htdoc folder,start Apache mysql.now run localhost/filename. ?>
...
<html>
    ...
    <body>
        <?php
            ...
            if ($push_type == 'topic') { // if push type is topic it will send everyone notification.
                $json = $push->getPush();
                $response = $firebase->sendToTopic('global', $json); //if we have mltiple different category(news- sports,entertainment,crime) notification at a time. it will be better not to put global. We can use specific category.other wise all notifications will show same time.

            } else if ($push_type == 'individual') {        //if we have specific device id .it will send individual notification.
                $json = $push->getPush();
